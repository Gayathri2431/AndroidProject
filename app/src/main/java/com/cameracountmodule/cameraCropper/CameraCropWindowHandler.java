 ...
 */
final class CameraCropWindowHandler {
     ...
     */
    public CameraCropWindowMoveHandler getMoveHandler(float x, float y, float targetRadius, CameraCropImageView.CropShape cropShape) {
        CameraCropWindowMoveHandler.Type type = cropShape == CameraCropImageView.CropShape.OVAL
                ? getOvalPressedMoveType(x, y)
                : getRectanglePressedMoveType(x, y, targetRadius);
        return type != null ? new CameraCropWindowMoveHandler(type, this, x, y) : null;
    }
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private CameraCropWindowMoveHandler.Type getRectanglePressedMoveType(float x, float y, float targetRadius) {
        CameraCropWindowMoveHandler.Type moveType = null;

        // Note: corner-handles take precedence, then side-handles, then center.
        if (CameraCropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.top, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.TOP_LEFT;
        } else if (CameraCropWindowHandler.isInCornerTargetZone(x, y, mEdges.right, mEdges.top, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.TOP_RIGHT;
        } else if (CameraCropWindowHandler.isInCornerTargetZone(x, y, mEdges.left, mEdges.bottom, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.BOTTOM_LEFT;
        } else if (CameraCropWindowHandler.isInCornerTargetZone(x, y, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.BOTTOM_RIGHT;
        } else if (CameraCropWindowHandler.isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom) && focusCenter()) {
            moveType = CameraCropWindowMoveHandler.Type.CENTER;
        } else if (CameraCropWindowHandler.isInHorizontalTargetZone(x, y, mEdges.left, mEdges.right, mEdges.top, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.TOP;
        } else if (CameraCropWindowHandler.isInHorizontalTargetZone(x, y, mEdges.left, mEdges.right, mEdges.bottom, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.BOTTOM;
        } else if (CameraCropWindowHandler.isInVerticalTargetZone(x, y, mEdges.left, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.LEFT;
        } else if (CameraCropWindowHandler.isInVerticalTargetZone(x, y, mEdges.right, mEdges.top, mEdges.bottom, targetRadius)) {
            moveType = CameraCropWindowMoveHandler.Type.RIGHT;
        } else if (CameraCropWindowHandler.isInCenterTargetZone(x, y, mEdges.left, mEdges.top, mEdges.right, mEdges.bottom) && !focusCenter()) {
            moveType = CameraCropWindowMoveHandler.Type.CENTER;
        }
     ...
     * @return the Handle that was pressed; null if no Handle was pressed
     */
    private CameraCropWindowMoveHandler.Type getOvalPressedMoveType(float x, float y) {
        ...
        */
        ...
        CameraCropWindowMoveHandler.Type moveType;
        if (x < leftCenter) {
            if (y < topCenter) {
                moveType = CameraCropWindowMoveHandler.Type.TOP_LEFT;
