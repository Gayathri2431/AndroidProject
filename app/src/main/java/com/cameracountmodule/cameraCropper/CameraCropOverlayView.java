 ...
 */
public class CameraCropOverlayView extends View {
     ...
     */
    private final CameraCropWindowHandler mCropWindowHandler = new CameraCropWindowHandler();
     ...
     */
    private CameraCropWindowMoveHandler mMoveHandler;
     ...
     */
    private CameraCropImageView.Guidelines mGuidelines;

    /**
     * The shape of the cropping area - rectangle/circular.
     */
    private CameraCropImageView.CropShape mCropShape;
    ...
    public CameraCropOverlayView(Context context) {
        this(context, null);
    }

    public CameraCropOverlayView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void resetCropOverlayView() {
        if (initializedCropWindow) {
            setCropWindowRect(CameraBitmapUtils.EMPTY_RECT_F);
            initCropWindow();
            invalidate();
        }
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public CameraCropImageView.CropShape getCropShape() {
        return mCropShape;
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public void setCropShape(CameraCropImageView.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17) {
                if (mCropShape == CameraCropImageView.CropShape.OVAL) {
     ...
     */
    public CameraCropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     */
    public void setGuidelines(CameraCropImageView.Guidelines guidelines) {
    ...
    public void setInitialCropWindowRect(Rect rect) {
        mInitialCropWindowRect.set(rect != null ? rect : CameraBitmapUtils.EMPTY_RECT);
        if (initializedCropWindow) {
            initCropWindow();
            invalidate();
     ...
     */
    public void setInitialAttributeValues(CameraCropImageOptions options) {
    ...
    private void initCropWindow() {
        ...
        float leftLimit = Math.max(CameraBitmapUtils.getRectLeft(mBoundsPoints), 0);
        float topLimit = Math.max(CameraBitmapUtils.getRectTop(mBoundsPoints), 0);
        float rightLimit = Math.min(CameraBitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottomLimit = Math.min(CameraBitmapUtils.getRectBottom(mBoundsPoints), getHeight());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == CameraCropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == CameraCropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        float left = Math.max(CameraBitmapUtils.getRectLeft(mBoundsPoints), 0);
        float top = Math.max(CameraBitmapUtils.getRectTop(mBoundsPoints), 0);
        float right = Math.min(CameraBitmapUtils.getRectRight(mBoundsPoints), getWidth());
        float bottom = Math.min(CameraBitmapUtils.getRectBottom(mBoundsPoints), getHeight());

        if (mCropShape == CameraCropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT >= 11 && Build.VERSION.SDK_INT <= 17 && mCropShape == CameraCropImageView.CropShape.OVAL) {
    ...
    private void drawCorners(Canvas canvas) {
        if (mBorderCornerPaint != null) {
            ...
            float w = cornerWidth / 2 + (mCropShape == CameraCropImageView.CropShape.RECTANGLE ? mBorderCornerOffset : 0);
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        float left = CameraBitmapUtils.getRectLeft(mBoundsPoints);
        float top = CameraBitmapUtils.getRectTop(mBoundsPoints);
        float right = CameraBitmapUtils.getRectRight(mBoundsPoints);
        float bottom = CameraBitmapUtils.getRectBottom(mBoundsPoints);
