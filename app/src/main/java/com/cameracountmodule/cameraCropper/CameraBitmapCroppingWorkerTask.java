 ...
 */
final class CameraBitmapCroppingWorkerTask extends AsyncTask<Void, Void, CameraBitmapCroppingWorkerTask.Result> {
     ...
     */
    private final WeakReference<CameraCropImageView> mCropImageViewReference;
     ...
     */
    private final CameraCropImageView.RequestSizeOptions mReqSizeOptions;
    ...
    CameraBitmapCroppingWorkerTask(CameraCropImageView cropImageView, Bitmap bitmap, float[] cropPoints,
                                   int degreesRotated, boolean fixAspectRatio, int aspectRatioX, int aspectRatioY,
                                   int reqWidth, int reqHeight, boolean flipHorizontally, boolean flipVertically, CameraCropImageView.RequestSizeOptions options,
    ...
    CameraBitmapCroppingWorkerTask(CameraCropImageView cropImageView, Uri uri, float[] cropPoints,
                                   ...
                                   boolean flipHorizontally, boolean flipVertically, CameraCropImageView.RequestSizeOptions options,
                                   Uri saveUri, Bitmap.CompressFormat saveCompressFormat, int saveCompressQuality) {
     ...
     */
    @Override
    protected CameraBitmapCroppingWorkerTask.Result doInBackground(Void... params) {
        try {
            if (!isCancelled()) {

                CameraBitmapUtils.BitmapSampled bitmapSampled;
                if (mUri != null) {
                    bitmapSampled = CameraBitmapUtils.cropBitmap(mContext, mUri, mCropPoints, mDegreesRotated, mOrgWidth, mOrgHeight,
                            mFixAspectRatio, mAspectRatioX, mAspectRatioY, mReqWidth, mReqHeight, mFlipHorizontally, mFlipVertically);
                } else if (mBitmap != null) {
                    bitmapSampled = CameraBitmapUtils.cropBitmapObjectHandleOOM(mBitmap, mCropPoints, mDegreesRotated, mFixAspectRatio,
                            mAspectRatioX, mAspectRatioY, mFlipHorizontally, mFlipVertically);
                } else {
                    return new Result((Bitmap) null, 1);
                }

                Bitmap bitmap = CameraBitmapUtils.resizeBitmap(bitmapSampled.bitmap, mReqWidth, mReqHeight, mReqSizeOptions);

                if (mSaveUri == null) {
                    return new Result(bitmap, bitmapSampled.sampleSize);
                } else {
                    CameraBitmapUtils.writeBitmapToUri(mContext, bitmap, mSaveUri, mSaveCompressFormat, mSaveCompressQuality);
                    if (bitmap != null) {
                        bitmap.recycle();
                    }
    ...
    protected void onPostExecute(Result result) {
        if (result != null) {
            ...
            if (!isCancelled()) {
                CameraCropImageView cropImageView = mCropImageViewReference.get();
