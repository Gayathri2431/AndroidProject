 ...
 */
public final class CameraBitmapUtils {
     ...
     */
    static Bitmap resizeBitmap(Bitmap bitmap, int reqWidth, int reqHeight, CameraCropImageView.RequestSizeOptions options) {
        try {
            if (reqWidth > 0 && reqHeight > 0 && (options == CameraCropImageView.RequestSizeOptions.RESIZE_FIT ||
                    options == CameraCropImageView.RequestSizeOptions.RESIZE_INSIDE ||
                    options == CameraCropImageView.RequestSizeOptions.RESIZE_EXACT)) {

                Bitmap resized = null;
                if (options == CameraCropImageView.RequestSizeOptions.RESIZE_EXACT) {
                    resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
                } else {
                    int width = bitmap.getWidth();
                    int height = bitmap.getHeight();
                    float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
                    if (scale > 1 || options == CameraCropImageView.RequestSizeOptions.RESIZE_FIT) {
